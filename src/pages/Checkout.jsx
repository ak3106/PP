    ...
    try {
      ...
      const orderData = {
        ...
        payment: {
          ...
          status: "pending", // later: paid / failed
        },
        ...
        status: "placed", // later: processing, shipped, delivered
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center min-h-[70vh]">
        ...
        <p className="text-gray-600 mb-6">
          Redirecting to login to verify your identity before purchase.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center min-h-[70vh] bg-green-50 rounded-3xl shadow-2xl">
        ...
        <p className="text-xl text-gray-700 mb-6">
          Thank you, {formData.name.split(" ")[0]}! Your order has been
          received.
        </p>
        <p className="text-gray-600 mb-8">
          We'll notify you once processing is complete.
        </p>
      ...
      {Object.keys(formErrors).length > 0 && (
        <div className="p-4 mb-6 bg-red-100 border-l-4 border-red-500 text-red-700 rounded-lg flex items-center shadow-md">
          ...
          <p className="font-semibold">
            Please correct the {Object.keys(formErrors).length} errors below
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100">
            ...
            <div className="space-y-4">
              ...
              <label
                className={`flex items-center p-4 rounded-xl cursor-pointer transition-all duration-200 border-2 ${paymentMethod === "cod" ? "bg-indigo-50 border-indigo-600" : "bg-gray-50 border-gray-200 hover:bg-gray-100"}`}
              >
                <input
                  type="radio"
                  name="payment"
              ...
              <label
                className={`flex items-center p-4 rounded-xl cursor-pointer transition-all duration-200 border-2 ${paymentMethod === "store" ? "bg-indigo-50 border-indigo-600" : "bg-gray-50 border-gray-200 hover:bg-gray-100"}`}
              >
                <input
                  type="radio"
                  name="payment"
                ...
                <div className="ml-4 flex items-center justify-between w-full">
                  <span className="font-semibold text-gray-800">
                    Pay at Store (Local Pickup)
                  </span>
                  <MapPin className="w-6 h-6 text-red-600" />
                </div>
                {isLocalPickup && (
                  <span className="ml-4 text-sm text-green-600 font-bold">
                    + FREE Shipping
                  </span>
                )}
              </label>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100">
            <h2 className="text-2xl font-bold text-primary mb-6 flex items-center">
              <Truck className="w-6 h-6 mr-3" /> 2. Delivery Information{" "}
              {isDelivery && (
            ...
            {isLocalPickup && (
              <div className="p-4 mb-4 bg-yellow-100 rounded-xl text-yellow-800">
                ...
                <p className="text-sm">
                  Please confirm your name and contact details below for
                  notification when your order is ready at the store (Madhya
                  Pradesh).
                </p>
              </div>
            )}
        ...
        <div className="lg:col-span-1 sticky top-24 h-fit">
          <div className="bg-indigo-50 p-6 rounded-3xl shadow-2xl border border-indigo-100">
            ...
            >
              ...
              {isPlacingOrder
                ? "Processing..."
                : `Place Order (${formatCurrency(GRAND_TOTAL)})`}
            </Button>

            <p className="mt-3 text-xs text-center text-gray-600">
              You are paying via{" "}
              {paymentMethod === "cod" ? "Cash on Delivery" : "Pay at Store"}.
            </p>
          </div>
        </div>
      </form>
